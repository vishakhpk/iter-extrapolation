...
def main():
    ...
    if data_args.dataset_name is not None:
        ...
        #with open(data_files['validation']) as f:
        #    lines = f.readlines()
        #    print(f[0])
    ...
    def compute_metrics(eval_preds):
        ...
        for i, label in enumerate(decoded_labels):
            ...
            temp_preds.append(decoded_preds[i])
        result = metric.compute(predictions=temp_preds, references=temp_labels)
        # result = metric.compute(predictions=decoded_preds, references=decoded_labels)
        result = {"bleu": result["score"]}

        prediction_lens = [np.count_nonzero(pred != tokenizer.pad_token_id) for pred in preds]
        result["gen_len"] = np.mean(prediction_lens)
        result["max_gen_len"] = float(np.max(prediction_lens))
